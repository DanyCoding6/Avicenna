...
import { bindActions, toast, sheet, confirm, emptyState } from '../ui.js';
import { cadence, personCard } from '../components/index.js';
import { refresh } from '../router.js';

export const header = { title: 'Coaching', backTo: '/programme' };
...
export async function render({ api }) {
  ...
  return `
    <div class="strand-hero"><h1 class="strand-hero__title">Coaching</h1><p class="strand-hero__sub">Two sessions a month with your coach. Fifty minutes, online, yours.</p></div>
  ...
  bindActions(root, {
    book: async () => {
      ...
      sheet({
        ...
        actions: [{ label: 'Confirm booking', onClick: async () => { if (!chosen) { toast('Pick a slot first', { type: 'error' }); return false; } try { await api.coaching.book(chosen); toast('Session booked'); refresh(); } catch (e) { toast(e.message, { type: 'error' }); refresh(); } } }],
