...
export async function render({ api, params }) {
  if (params.id) {
    ...
    return `<article class="journal-entry">
      <div class="cover girih" style="margin:16px var(--gutter) 0"></div>
      ...
      ${j.gallery ? `<div class="gallery">${Array.from({ length: j.gallery }, (_, i) => `<div class="cover girih" style="opacity:${1 - i * 0.12}"></div>`).join('')}</div>` : ''}
